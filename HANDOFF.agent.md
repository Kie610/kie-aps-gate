...
not-run:
- U: **実験フラグの実機 A/B round 2** (ユーザーのみ実施可能)。手順:
  1. `kieApsGate` コンポーネントで `immobilizeClonePhysBones` だけ ON → アップロード →
     体固定 + PB 固定解除 → その場回転・接近・周回・急停止で固定体の髪を見る
  2. 直れば案1採用 (凍結フラグは削除候補)。直らなければ `freezeClonePbWhileMoving` も
     ON → 移動中に凍結されること・停止後に自然へ戻ることを確認
  3. どちらも不発なら kieApsGate では塞げない結論を確定 → VRChat SDK へフィードバック
     (最小再現: 素のアバター + MA World Fixed Object の箱 + 髪チェーン PB 1 本、
     その場回転で流れる)。案1 が効いた場合は APS 作者へ「APS_PB の Immobile 動的切替」を
     提案できる (本来 APS 本体でやるのが正しい層)
