...
> **注意: 0.3.0-alpha で効果が下がった。数値は再計測待ち。**
>
> 0.2.0-alpha までは PhysBone を含むサブツリーまで落としており、**固定した瞬間に
> 揺れものがレスト位置で固まる**不具合があった。これを直したため、ゲートできる
> constraint が実測で **48 個 → 10 個**に減っている。
> 下の数値は 0.2.0-alpha 時点のもので、**現在は当てはまらない**。
